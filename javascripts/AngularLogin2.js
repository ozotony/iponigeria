...
app.factory('dataFactory', ['$http','$q', function ($http, $q) {
    ...
    dataFactory.checkemail = function (vemail) {
        ...
        var url7 = 'http://ipo.cldng.com/Handlers/EmailCount.ashx';
        var vresult=true;
        // var kkk = $('#VEMAIL').val();
        var kkk = vemail;
...
app.controller('myController', ['$scope', '$http', '$rootScope', 'authService2', '$location', '$q', function ($scope, $http, $rootScope, authService2, $location, $q) {
    ...
    $scope.$on('$viewContentLoaded', function () {
        ...
        { $rootScope.greeting = "Good Afternoon";  }
        if ((h >= 18) && (h <= 23))
...
function (isConfirm) {
    if (isConfirm) {
        $.ajax({
            ...
            url: 'http://ipo.cldng.com//Handlers/a_login2.ashx',
...
app.controller('myController3', function ($scope, $http, $rootScope, $routeParams) {
    ...
    $scope.Update2 = function () {
        $rootScope.vhide = false;
        var url3 = 'http://ipo.cldng.com/Handlers/BranchCollect3.ashx';
...
app.controller('myController4', function ($scope, $http, $rootScope, $routeParams,dataFactory) {
        ...
        if (isValid) {
           ajaxindicatorstart('Submitting   Record.. please wait..');
            ...
            for (var i = 0; i < totalFiles3; i++) {
                ...
                formData.append("vv", JSON.stringify(AgentsData));
            //    ajaxindicatorstart('Submitting Record.. please wait..');
                var url7 = 'http://ipo.cldng.com/Handlers/SaveAgent.ashx';
                //$.ajax({
                //    type: "POST",
                //    url: url7,
                //    data: formData,
                ...
                //    }
                //});
                var url9 = "http://ipo.cldng.com/Handlers/SaveAgent.ashx";
    ...
    $scope.GetStates = function () {
        ...
        if (countryId) {
            ...
            $.ajax({
                type: "POST",
                url: 'http://ipo.cldng.com/Handlers/GetState.ashx',
                data: formData,
...
app.controller('myController6', function ($scope, $http, $rootScope, $routeParams, $location) {
    ...
    $scope.$on('$viewContentLoaded', function () {
        ...
        $http({
            method: 'GET',
            url: 'http://ipo.cldng.com/Handlers/GetRegistration2.ashx'
...
function IpoTradeMarks(email,password) {
    ...
    postwith('http://ipo.cldng.com/a_login.aspx', {
        x3: email,
        x2: password
    });
...
function IpoTradeMarks2(email, name, address, vid, PhoneNumber) {
    ...
    postwith('http://88.150.164.30/Payx/A/m_payx.aspx', {
